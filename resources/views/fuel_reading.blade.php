                ...
                <div class="col_70">
                    ...
                    <div class="pump_notice">
                        ...
                        <form class="my_form" action="{{action('FuelController@store')}}" method="POST">
                            ...
                            <div class="view_openings_div">
                                <a href="/view_openings"><button class="new_btn color8 bg1" type="button">View Openings&nbsp;<i class='bx bx-log-in icon'></i></button></a>
                            </div>
                            <h4><i class='fa fa-calculator'></i>&nbsp;&nbsp;All Pumps</h4>
                            @foreach ($pumps as $item)
                                ...
                                @if ($item->fuel_type == 'both') 
                                    ...
                                    @if ($openings != '')
                                        @if ($openings->$psc == 0)value="{{$openings->$pd}}" @else value="{{$openings->$pdc}}" @endif
                                    @endif 
                                    type="number" min="0" step="any" class="form-control" placeholder="Face2 Opening #" readonly>

                                    <input class="cls" name="xpump{{$item->id}}s1" 
                                    {{-- @if ($openings != '') value="{{$openings->$psc}}" @endif --}}
                                    type="number" min="0" step="any" class="form-control" placeholder="Face1 Closing #" required>   

                                    <input class="cls" name="xpump{{$item->id}}d1" 
                                    {{-- @if ($openings != '') value="{{$openings->$pdc}}" @endif --}}
                            ...
                            @foreach ($tanks as $item)
                                ...
                                @if ($openings != '')
                                    @if ($openings->$xtk == 0)value="{{$openings->$tk}}" @else value="{{$openings->$xtk}}" @endif
                                @endif 
                                type="number" min="0" step="any" class="form-control" 
                                placeholder="Input {{$item->tank_name}} opening value" readonly>
                            ...
                            <div class="btn_grp">
                                <button type="submit" class="submit_btn" name="store_action" value="update_opening"><i class="fa fa-save"></i>&nbsp;&nbsp;Update Readings</button>
